...
summary: Three independent clients obey the same rule set in three languages. They share no library, no backend, and no database — only the contracts defined by the access rule and the entity shape.
part: Surfaces
folio: 'III.03'
order: 3
status: settled
