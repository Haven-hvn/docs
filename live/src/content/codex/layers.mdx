...
**DFINITY ICP** runs the access checks. Canister `gny6k-fqaaa-aaaab-ag3ra-cai` performs VetKD key derivation, EIP-712 signature recovery, and EVM balance checks. It is the only component that touches threshold cryptographic material, and it does so without assembling a full key.
